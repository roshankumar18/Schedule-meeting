...
export async function POST(request: NextRequest) {
  try {
    ...
    const meetings = await db
      .select()
      .from(meeting)
      .where(or(eq(meeting.user1, user2), eq(meeting.user2, user2)));
    if (!availabilityAndPrefrence) {
      return Response.json(
        { error: "Availability or preference data not available." },
        { status: 404 },
      );
    }
    const {
      ...
      availabilty: schema.Availability[] | null;
      prefrence: schema.Prefrence;
    } = availabilityAndPrefrence;
    ...
    if (maxMeetingsPerDayNumbers[day] === meetings.length) {
      ...
      return Response.json(
        { error: "Maximum meetings per day limit reached." },
        { status: 403 },
      );
    }
    const isTimeBooked = meetings.some((meeting) => {
      const meetingStart = DateTime.fromFormat(
        meeting.startTime,
        "h:mma",
      ).minus({ minutes: Number(prefrence.before) });
      const meetingEnd = meetingStart
        .plus({ hours: 1 })
        .plus({ minutes: Number(prefrence.after) });
    ...
    if (isTimeBooked) {
      ...
      return Response.json(
        { error: "The proposed meeting time is already booked." },
        { status: 409 },
      );
    }
    const availableSlots = availabilty.filter((slot) => {
      return (
        slot.day === day &&
        DateTime.fromFormat(slot.startTime, "h:mma") <=
          DateTime.fromFormat(meetingTime, "h:mma") &&
        DateTime.fromFormat(slot.endTime, "h:mma") >=
          DateTime.fromFormat(meetingTime, "h:mma").plus({ hours: 1 })
      );
    });
    ...
    if (availableSlots.length > 0) {
      await db
        .insert(meeting)
        .values({ user1, user2, day, startTime: meetingTime });
      return Response.json(
