...
interface PrefrenceProps {
  editSchedule : () =>void,
  submitPreference : (before:string,after:string,maxMeetings:string[]) =>void
}
...
const Prefrence = ({editSchedule,submitPreference}:PrefrenceProps) => {
  ...
  const [error,setError] = useState<{before:string,after:string,meetings:string}>({
    before:'',
    after:'',
    meetings:''
  })
  ...
  const handleMaxMeetingsChange = (e:React.ChangeEvent<HTMLInputElement>, index:number) => {
      ...
      <form onSubmit={onSubmitHandler}>
        ...
        <div>
          ...
          {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map((day, index) => (
            <div key={day} className='flex items-center justify-between gap-2 mt-2'>
              <label >{day}</label>
              <input
              className='border-2 border-slate-500'
                type="number"
        ...
        {error.meetings && <div className='text-red-600'>{error.meetings}</div>}
          <button type='submit' className='bg-blue-400 p-2 mt-4  w-52 hover:bg-blue-600 self-center'>Submit</button>
        
        
      </div>

      </form>
      <button onClick={editSchedule} className='bg-blue-400 p-2 mt-4  w-52 hover:bg-blue-600 self-center'>Go Back</button>
    </div>
