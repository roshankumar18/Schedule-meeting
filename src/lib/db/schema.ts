...
import { integer, jsonb, pgEnum, pgTable, primaryKey, serial, timestamp, uniqueIndex, varchar } from 'drizzle-orm/pg-core';
...
export const meeting = pgTable('meeting',{
  id: serial('id').primaryKey(),
  user1:varchar('user1',{length:256}).notNull().references(()=>users.user_id),
  user2:varchar('user2',{length:256}).notNull().references(()=>users.user_id),
  day:varchar('day',{length:256}).notNull(),
  startTime:varchar('startTime',{length:256}).notNull(),
})
